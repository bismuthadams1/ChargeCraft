    ...
    def _hash(cls, instance: DDXSettings) -> int:
        return hash(
            (
                instance.ddx_model,
                instance.solvent,
                _float_to_db_int(instance.epsilon) if instance.epsilon is not None else None,
                instance.radii_set
            )
        )
...
class DBConformerPropRecord(DBBase):
    ...
    alpha_density = Column(PickleType, nullable=False)
    beta_density = Column(PickleType, nullable=False)
